  ...
  <div class="control-section">
    <div id="spreadsheet-default-section">
      ...
      <div class="spreadsheet-buttons-container">
        ...
        <button cssClass="e-control" v-on:click="copyDataOnly">
          Copy Data Only
        </button>
        <button cssClass="e-control" v-on:click="copySelection">
          Copy Selection
        </button>
      </div>
      <ejs-spreadsheet
      ...
      >
        <e-sheets>
          <e-sheet
            ...
            :isProtected="spreadsheetProtected"
...
export default Vue.extend({
  ...
  methods: {
    ...
    buildTotalsRowFormulaConfigs() {
      ...
      for (let i = firstLetterIndex; i <= lastLetterIndex; i++) {
        ...
        configs.push({
          ...
          props: {
            formula: `=SUM(${letter}2:${letter}${
              this.spreadsheetDataSource.length + 1
            })`,
            isLocked: false,
          },
        });
    ...
    styleSpreadsheet() {
      ...
      spreadsheet.cellFormat(
        { textAlign: "center", fontSize: FONT_SIZE_DATA, fontFamily: "Arial" },
        `A1:Z${lastRowIndex}`
      );
      spreadsheet.cellFormat(totalsConfig, `Z2:Z${lastRowIndex}`);
      spreadsheet.cellFormat(
        totalsConfig,
        `B${lastRowIndex}:Z${this.spreadsheetDataSource.length + 2}`
      );
      spreadsheet.numberFormat(
        "0.###",
        `B2:Z${this.spreadsheetDataSource.length + 2}`
      );
      spreadsheet.cellFormat(
        { backgroundColor: "#f5f0ee", textAlign: "left" },
        `A2:A${this.spreadsheetDataSource.length + 1}`
      );

      spreadsheet.cellFormat(
        { fontSize: "10px" },
        `B${2}:Z${this.spreadsheetDataSource.length + 2}`
      );
